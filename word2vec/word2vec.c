...
struct vocab_word {
  //cn 词的数量 word 词内容 point 根节点到子节点的路径 code huffman编码 codelen 编码长度
  long long cn;
  int *point;
...
//syn0 词向量，初始的词向量是随机值。计算完成之后，输出，作为最终的词向量
//syn1 用于Hierarchical Softmax 方式的计算 也就是输出
//syn1neg 用于 NEGATIVE SAMPLING 计算
...
void *TrainModelThread(void *id) {
  ...
  while (1) {
    ...
    if (sentence_length == 0) {
      while (1) {
        //读取文件的每个词，并且在初始化的词汇表，查询得到编码，也就是huffman码
    ...
    if (cbow) {  //train the cbow architecture
      ...
      if (cw) {
        ...
        //负采样 (默认采用这种方式)
        //随机选择 negative 个词，如果命中 word 则不更新误差；否则更新误差 更新网络参数
        // NEGATIVE SAMPLING
